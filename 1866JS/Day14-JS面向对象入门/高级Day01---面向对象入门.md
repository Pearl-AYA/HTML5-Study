...
1）面向过程的思路：打地基、砌墙、盖屋顶、装修、安装水电

2）面向对象的思路：打地基、砌墙——> 泥瓦匠类：泥瓦匠对象---会打地基、会砌墙       男性、不过敏

砌匠类：砌匠师傅   ——>  盖屋顶

装修工类：赋予他们装修的功能   

安装水电：水电工类：水电工对象     安装水管、电
...
面向对象的语言：Java   C#   C++   Objective-C、JS面向对象
...
- 多态
  ...
  #### 不同实例(对象)的相同方法在不同情形下有不同的表现形式。多态机制使具有不同内部结构的对象可以【共享相同的外部接口】。
...
2. 构造函数和普通函数仅仅也仅仅是调用方式的不同。也就是说，随便一个函数你如果用new 的方式去使用，那么他就是一个【构造函数】。

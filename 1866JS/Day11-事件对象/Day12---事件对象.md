...
​	在触发 DOM 上的某个事件时，会产生一个【事件对象 event 】，这个对象中包含着所有与事件有关的
...
<body>
    ...
    <script type="text/javascript">
	    var btn = document.getElementById("btn");
      	//当点击事件发生的时候，浏览器会把发生的这次点击封装成一个事件对象，通过事件处理程序的参数传进来。
      	//在处理程序内部就可以使用这个事件对象了。
...
​	当一个页面中需要给比较多的元素添加事件处理程序的时候，以前的做法是通过找到每个元素，然后逐一的去添加。这样做有一些缺点：

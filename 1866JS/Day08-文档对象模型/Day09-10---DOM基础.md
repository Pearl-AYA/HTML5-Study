...
2. Node.ATTRIBUTE_NODE (2)   //元素的属性<P id="hello" title="招呼">HELLO</P>
...
9. Node.DOCUMENT_NODE (9) //最外层的Root element,包括所有其它节点    根节点   document文档节点
...
12. Node.NOTATION_NODE (12) //DTD中的Notation定义    在XML文档中表示一个符号。
...
自 Document 类型）的一个实例，表示**整个 HTML 页面**。而且， document 对象是 window 对象的一个属性，因此可以将其作为全局对象来访问。
...
## 2.3	获取节点方法3：getElementsByName(name)==(083)==
...
4. 根  (root)  节点     就是document节点，一个文档只能有一个根节点。对html文档来说，根元素节点就是documentElement。根节点不可能有父节点，但是根元素节点的父节点是document节点。根节点就是document节点。
...
> tagName属性是nodeName的另一种写法。注意区别是：tagName只在标签节点中有，而nodeName在所有类型的(12种)节点中都存在。
>
> 它的值表示的就是标签的名字。

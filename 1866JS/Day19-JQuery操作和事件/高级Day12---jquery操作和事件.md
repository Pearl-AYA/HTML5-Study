...
​	CSS-DOM是针对CSS的操作。主要作用就是获取数组style对象的各种属性。让网页呈现出不同的效果。
...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function () {
				//获取得到的标签的内的文本，不包括内部的标签。
                  //但是内部标签与标签中的空格会获取到
				var text = $(".box").text();//类似ele.innerText
				console.log(text);
				var html = $(".box").html();
				//标签内的所有内容，包括子标签本身   类似ele.innerHTML
				console.log(html);
				
				//上面的方法不传入参数表示只是获取，
          	     //如果传入字符串表示设置
				//$(".box").text("哈哈哈");
				//设置html，如果有标签，则会自动解析出来
	...
	<head>
		...
		<script type="text/javascript">
			$(function() {
				...
				$("div button").click(function () {
					 $("div a")
					.attr("href", "http://www.sina.com.cn") //修改
                      //参数1：属性名  参数2：属性值
					.attr("title", "浪浪") //添加并设置title属性
					.text("浪一下");//修改
				})
			})
		</script>
...
#### 	这里的样式操作主要是【针对一个元素的class属性进行操作】。一般有一些潜规则：id是给js用的，class是给css用的。所以，大家希望用class去设置样式。

#### 	对样式的操作包括：获取class，设置class，追加class，删除class，切换class，判断是否有class属性

### 2.3.1	获取和设置样式

> class也是属性，所以获取和设置的方法与【获取和设置属性的方法一致】。
...
​	【替换样式】会把原来的样式全部更新，有的时候，我们【只想更新】某个样式，则可以使用追加。

> CSS规定，一个元素的class属性可以设置多个值，多个值之间用空格隔开。表示合并这些样式的值。如果有重复的，则以后面的为准
>
...
$(function () {
    $("li:last").click(function () {
      	//该方法会自动判断这个class(指的是class为color)是否存在，
        //如果存在就删除，不存在就添加
...
<script type="text/javascript">
	$(function () {
  		//遍历选择到所有元素。传入的参数中有两个参数：
         //参数1：遍历的元素的下标index
     	 //参数2：遍历到的那个元素(dom对象，不是jq对象)
...
​	获取这个元素相对于【当前文档的相对偏移】。其中返回的对象包含两个属性，left和top     这里的这个offset相当于前面我们学过的pageX和pageY。
		...
		<script type="text/javascript">
			$(function() {
				$("button:eq(0)").click(function() {
					var top = $("p:first").scrollTop();
                      //获取垂直滚动条距离顶端的距离
					var left = $("p:first").scrollLeft();
                      //获取水平滚动条距离左端的距离
				...
				$("button:eq(1)").click(function() {
					timerId = setInterval(function() {
						var top = $p.scrollTop();
						$p.scrollTop(top + per);
                          //更改垂直滚动条距离顶端的距离，则滚动条滚起来
					}, 50);//每隔50ms滚动一次，每次增加per的px像素
				})
...
```javascript
var $li1 =$("<li>JavaScript</li>");	
//创建一个li节点和一个文本节点。元素节点和文本节点一起创建的
```
	...
	<head>
		...
		<script type="text/javascript">
			$(function () {
				...
				//清空找到的节点的所有内容，包括所有后代节点。
                 //父节点还在。  这个其实并没有清除父节点，专清空后代节点
...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function () {
				$(".btns button:eq(0)").click(function () {
				//用div把 .languages 包裹起来。  
                  //如果多个元素需要包裹，则会有多个div
//					$(".languages").wrap("<div></div>");
					//所有的用一个div包裹起来。  如果中间有其他元素，
                  //其他元素则放置包裹之后
//					$("ul").wrapAll("<div></div>");
...
#### 	JQuery对象提供了两个合成事件：hover和toggle

1. hover(f1, f2)用于模拟光标悬停事件，当光标移动到元素上的时候，会触发指定的第一个函数，当光标移除这个元素时，会触发第二个函数
2. 功能1：toggle(f1, f2, ... fn) 用于模拟鼠标连续单击事件，当第一次单击时执行第一个函数，当第二次单击执行第二个函数，...，执行完毕之后再循环来。功能2：不传任何参数，可以让元素隐藏或显示。   但是从1.9版本开始，只剩下了功能2，功能1已经被干掉了。
...
<script type="text/javascript">
	$(function() {
      	//匿名函数的参数就是事件对象。执行的时候浏览器
        //会自动创建事件对象，并传入函数中。
      	//这里的【事件对象】已经经过了JQuery的封装。是【JQuery对象】
...
5. pageX和pageY属性    光标相对于顶端页面的距离。如果有滚动条，还要加上滚动条的距离
6. offsetX和offsetY   触发点距离当前元素的边缘的距离
7. clientX和clientY    触发点距离浏览器边缘的距离
8. which属性。 如果是鼠标事件则获取鼠标的左(1)中(2)右(3)键，如果是键盘则获取【键盘的按键】
...
##### 	preventDefault()阻止默认行为，stopPropation()阻止冒泡。  事件函数中返回false，可以同时停止默认行为和冒泡。所以，可以直接返回false，而不用调用上述两个方法(这个很好很强大)。
...
3. **执行默认操作**：trigger触发的事件，不仅仅触发了事件函数，也会触发浏览器的默认操作。比如，给input注册一个focus事件，则不但触发focus事件，还会触发浏览器的默认事件，使input获得焦点。使用triggerHanddler可以避免浏览器的默认操作(即避免输入框的边缘默认变色等)
...
​	JQuery为了方便管理事件，给事件提出了**命名空间**的概念。   

...
> 一、Symbol.for(字符串参数)：在全局环境中搜索 以该字符串作为参数的Symbol值，如果搜到则返回这个symbol，如果搜不到则创建一个Symbol，并把它注册在全局环境中。Symbol(”s“)
...
《数据结构》队列    栈   二叉树  红黑树   结构体  联合体（共用体） 枚举

队列：先进先出，后进后出     例子：排队   只能操作队尾

栈：先进后出，后进先出    例子：拿薯片     栈底是封闭的，栈顶是开放的
...
> **生成器函数就是【返回迭代器的函数】**
>
> 【生成器函数】由 function 关键字和之后的星号（*）标识，同时还能使用新的 yield 关键字。
...
1. 你只能在派生类中使用 super()，否则（如果在没有使用 extends 的类或函数中使用）一个错误会被抛出。

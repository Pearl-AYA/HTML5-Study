	...
	query.fn = query.prototype = {
		...
		init : function (selector) {
			if (typeof selector == 'string') {
				//这里是给this对象添加一个叫做【element的属性】
				//这个element不是window上的element
				//所以在外面使用element的时候必须先导入这个touch.js文件
			...
			function touchFn (e) {
				switch (e.type) {
					...
					case 'touchend':
						...
						//3、这个【Math.abs(endX - startX) >= Math.abs(endY - startY)】条件
						//只表明有左右滑动，还不能确定具体是【左滑】还是【右滑】
						if (Math.abs(endX - startX) >= Math.abs(endY - startY) 
								&& (startX - endX) >= 25) {
							// 【归纳】开始的位置startX - 结束的位置endX >= 0表示是【左滑】
							// 25px表示【容错范围值】
							handler();
						}
						break;
			...
			function touchFn (e) {
				switch (e.type) {
					...
					case 'touchend':
						...
						if (Math.abs(endX - startX) >= Math.abs(endY - startY) 
								&& (endX - startX) >= 25) {
							// 【归纳】结束的位置endX - 开始的位置startX >= 0表示是【右滑】
							// 25px表示【容错范围值】
							handler();
						}
						break;
